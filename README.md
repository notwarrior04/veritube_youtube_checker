...
**VeriTube** is an end-to-end multimodal AI application designed to verify YouTube video content in real-time. It automatically transcribes audio, evaluates title sensationalism (clickbait/misleading scoring), extracts testable factual claims, and cross-verifies claims using **Google Gemini 1.5 Flash** or an **Edge AI Local Pipeline** (Spacy, SentenceTransformers, BART NLI, Wikipedia, and Google Serper Search).
...
- **🧠 Dual-Engine Fact Checking**:
  - **Cloud AI Engine**: Powered by **Google Gemini 1.5 Flash** for real-time web-grounded fact verification.
...
flowchart TD
    ...
    F --> G["5. Extract Factual Claims (Gemini 1.5 Flash / spaCy)"]
    G --> H{"Check Claims against Sources"}
    
    H -- "Cloud AI Engine" --> I["Fact Verification (Gemini 1.5 Flash + Web Grounding)"]
    H -- "Edge AI Engine" --> J["Fact Verification (Serper.dev Search + Wikipedia API + BART NLI)"]
    
    I --> K["6. Summarization & Verdict Assembly (Gemini 1.5 Flash / BART-Large-CNN)"]
    J --> K
    E --> K
...
└── backend/
    ...
    ├── gemini_engine.py       # Google Gemini 2.5 Flash AI integration
...
| **Cloud AI Engine** | `gemini_engine.py` | Real-time web-grounded verification via Gemini 2.5 Flash. |
